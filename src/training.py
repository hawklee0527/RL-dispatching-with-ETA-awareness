...
class Learning(nn.Module):
    ...
    def act(self, req, avail_vehs, debug_plot=False):
        """ Two-step action:
    ...
    def save_model(self, save_path, backup=True):
    ...
    def train(self, data, tolerance=50, per_epis_depict=1000):
        ...
        for episode in range(self.max_episode): # do an episode
            ...
            global RH
            RH = RollingHorizon(data, vehi_depot, steps=steps)
            ov_epi = RH.main()
            ...
            print(f'Instance [{inst.replace(".parquet", "")}] {inst_idx}/{len(insts)}')
            ...
            if recent_mean > self.recent_best:
                ...
                recent_fail += 1
                if episode > 1000 and recent_fail >= tolerance:
                    break
...
class RollingHorizon():
    def __init__(self, data, vehi_depot, steps=5):
        # data
        self.data = data.copy()
    ...
    def main(self): 
        ...
        while self.now <= self.T:
            ...
            for _, req in It.iterrows():
                ...
                car_idx, eta, log_prob, state_value = RL.act(req, range(K))
                is_dummy = True if car_idx == K else False
                applied, Δov = self.apply(req, car_idx, eta, is_dummy=is_dummy)
                self.update_heads()
                
                # back propagation
                RL.update(log_prob, state_value, Δov, is_dummy=is_dummy)
            self.now += self.Δt  
...
inst_dir = 'D://Speed-optimization/code/Github code/Data/Training/Instance 100/'  # direct to data's documentory
save_dir = 'D://Speed-optimization/code/Github code/Model/Instance 100/'          # path for saving
insts = os.listdir(inst_dir)

for inst_idx, inst in enumerate(insts):  # patch training instances in dataset
    # RL save & load paths
    inst_path = inst_dir + inst
    save_path = save_dir + inst.replace('.parquet', '.pth')
    
    if inst_idx > 0:
        load_path = save_dir + insts[inst_idx-1].replace('.parquet', '.pth')
    else:
        load_path = None
